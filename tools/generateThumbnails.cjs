...
function generateMapThumbnail(mapData, width = 200, height = 150) {
  ...
  // Title
  svgContent += `<text x="${width/2}" y="20" text-anchor="middle" fill="white" font-family="Arial" font-size="12" font-weight="bold">${mapData.name}</text>`;
...
function generateAllThumbnails() {
  ...
  for (const mapFile of mapFiles) {
    try {
      ...
      console.log(`✓ Generated thumbnail for ${mapData.name}: ${thumbnailName}`);
    } catch (error) {
      console.error(`✗ Error generating thumbnail for ${mapFile}:`, error.message);
    }
  }
}
